	...
	protected void paintComponent(Graphics g) {
		...
		if (type == Type.SIMPLE_INDICATOR) {
			...
			for (int i = 0; i < values.size(); i++) {
				g2d.setColor((Color) colors.get(i));//g2d.setColor(colors.get(i));

				Double val = (Double) values.get(i);//Double val = values.get(i);
			...
			for (int i = 0; i < gradingValues.size(); i++) {
				g2d.setColor((Color) gradingColors.get(i));//g2d.setColor(gradingColors.get(i));
				Double val = (Double) gradingValues.get(i);//Double val = gradingValues.get(i);
